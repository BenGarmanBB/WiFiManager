    ...
    for (int i = 0; i < _paramsCount; i++) {
     ...
     String pitem;
      ...
      if (_params[i]->getID() != NULL) {
        ...
        if(tok_c)pitem.replace(FPSTR(T_c), String(String(_params[i]->getCustomHTML()) + (_params[i]->getValueLength() < 10) ? String(_params[i]->getValue()) : "").c_str());// == "checked" ? " checked" : "hi")).c_str()); // T_c meant for additional attributes, not html, but can stuff
      } else {
